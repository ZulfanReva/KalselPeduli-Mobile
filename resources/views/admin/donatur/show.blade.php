                ...
                <div class="flex flex-row gap-x-16">
                    ...
                    <div class="flex flex-col gap-y-10">
                        ...
                        @if ($donatur->sudah_dibayar)
                            <span class="w-fit text-sm font-semibold py-2 px-4 rounded-full bg-green-500 text-white">
                                Aktif
                            </span>
                        @else
                            <span class="w-fit text-sm font-semibold py-2 px-4 rounded-full bg-orange-500 text-white">
                                Menunggu
                            </span>
                        @endif
                        <div>
                        ...
                        <div>
                            ...
                            <h3 class="text-indigo-950 dark:text-gray-200 text-xl font-bold">{{ $donatur->nama }}</h3>
                        </div>
                    </div>
                    <div>
                        ...
                        <h3 class="text-indigo-950 dark:text-gray-200 text-xl font-bold">
                            {{ $donatur->proyekPenggalangan->nama }}</h3>
                        <p class="text-slate-500 dark:text-gray-400 text-sm"> Target Donasi
                            {{ 'Rp ' . number_format($donatur->proyekPenggalangan->target_donasi, 0, ',', '.') }}</p>
                    </div>
                </div>
                ...
                @if (!$donatur->sudah_dibayar)
                    ...
                    <div class="flex justify-end">
                        <form action="{{ route('admin.donatur.update', $donatur->id) }}" method="POST">
                            ...
                            @method('PUT')
                            <button type="submit"
                                class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full hover:bg-indigo-600 transition duration-200 ease-out">
                                Terima Donasi
                            </button>
                        </form>
                    </div>
